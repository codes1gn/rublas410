...
fn div_f32(crit: &mut Criterion) {
    let mut bench_group = crit.benchmark_group("div_f32");
        ...
        for Ksize in vec![16, 256, 1024, 4096].iter() {
            ...
            bench_group.bench_with_input(
                ...
                |bench, msize| {
                    ...
                    bench.iter(|| {
                        ...
                        black_box(exec.divf32_owned(lhs, rhs));
                    });
                },
            );
            ...
            bench_group.bench_with_input(
                ...
                |bench, msize| {
                    ...
                    let exec = BlasExecutor::new();
                    bench.iter(|| {
                        black_box(exec.divf32_side_effect(&lhs, &rhs, &mut out));
                    });
                },
            );
        }
...
criterion_group!(div_tests, div_f32);
criterion_main!(div_tests);
